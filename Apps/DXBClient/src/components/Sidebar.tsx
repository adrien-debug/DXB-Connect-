        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className={`p-5 flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
            {!collapsed && (
              <div className="flex items-center gap-3 animate-fade-in-up overflow-hidden">
                <div className="relative flex-shrink-0">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-lime-400">
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-zinc-500 hover:text-white transition-all ease-hearst duration-300 flex-shrink-0 border border-white/5"
            >
          ...
          <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto">
            {navItems.map((item, index) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-zinc-950 rounded-r-full" />
                  )}
                  ...
                  `}>
                    <Icon size={20} className={isActive ? 'text-zinc-950' : 'text-current'} />
                  </div>
                  ...
                  {!collapsed && (
                    <div className="flex-1 min-w-0">
                      ...
                      <span className={`text-[10px] truncate block ${isActive ? 'text-zinc-950/60' : 'text-zinc-600'}`}>
                        {item.description}
                      </span>
                    </div>
                  )}
                  ...
                  {!collapsed && !isActive && (
                    <ChevronRight
                      size={16}
                      className="opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all ease-hearst duration-300 text-zinc-600"
                    />
                  )}
                </Link>
              )
          ...
          <div className="px-3 pb-3">
            <button
              ...
              `}
            >
              <div className="relative flex items-center justify-center w-10 h-10 rounded-lg bg-lime-400/20">
                <ShoppingCart size={18} />
                {itemCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-lime-400 rounded-full text-[10px] font-bold text-zinc-950 flex items-center justify-center">
              ...
              {!collapsed && (
                <div className="flex-1 text-left">
                  ...
                  <span className="text-[10px] text-lime-400/60">{itemCount} article(s)</span>
                </div>
              )}
            </button>
          </div>
            ...
            {!collapsed && user && (
              <div className="px-3 py-3 rounded-xl bg-white/5 border border-white/5 mb-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center text-zinc-950 text-sm font-bold bg-lime-400">
